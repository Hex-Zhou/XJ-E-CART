...
export class PageCategoryComponent implements OnDestroy {
    ...
    this.route.data.subscribe(data => {
      ...
      if (!this.getCategorySlug()) {
        ...
        this.breadcrumbs = this.breadcrumbs.concat([
          ...data["category"].parents.map((parent: Category) => ({
            label: parent.name,
    ...
    this.pageService.optionsChange$
      .pipe(
        ...
        mergeMap(() => {
          ...
          return this.shop
            .getProductsList(this.getCategorySlug(), this.pageService.options)
  ...
  getCategorySlug(): string | null {
    return (
      this.route.snapshot.params["categorySlug"] || this.route.snapshot.data["categorySlug"] || null
    );
  }
