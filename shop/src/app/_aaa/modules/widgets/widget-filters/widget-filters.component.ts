...
export class WidgetFiltersComponent implements OnInit, OnDestroy {
      ...
      .subscribe(filters => {
        ...
        filters.forEach((filter: any) => {
          switch (filter.type) {
            case "range":
              merge(
                of([Math.max(filter.value[0], filter.min), Math.min(filter.value[1], filter.max)]),
  ...
  isItemDisabled(
    filter: CheckFilter | RadioFilter | ColorFilter,
    item: FilterItem | ColorFilterItem
  ): boolean {
    return item.count === 0 && (filter.type === "radio" || !filter.value.includes(item.slug));
  }
  convertFormToFilterValues(
    filters: Filter[],
