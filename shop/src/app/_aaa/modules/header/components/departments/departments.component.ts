...
import { Subject, merge, takeUntil, fromEvent, filter } from "rxjs";
import { fromMatchMedia } from "src/app/shared/functions/rxjs/fromMatchMedia";
import { fromOutsideTouchClick } from "src/app/shared/functions/rxjs/fromOutsideTouchClick";
import { NavigationLink } from "src/app/shared/interfaces/navigation-link";
import { HeaderService } from "src/app/shared/services/header.service";
import { departments } from "src/data/header-departments";
@Component({
...
export class DepartmentsComponent implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    if (isPlatformBrowser(this.platformId)) {
      ...
      fromMatchMedia("(min-width: 992px)")
        .pipe(
  ...
  ngAfterViewInit(): void {
    if (isPlatformBrowser(this.platformId)) {
      this.zone.runOutsideAngular(() => {
        fromOutsideTouchClick(this.element)
          .pipe(takeUntil(this.destroy$))
          .subscribe(() => {
            if (this.isOpen) {
  ...
  ngAfterViewChecked(): void {
    ...
    if (this.hoveredItem?.menu?.type === "menu") {
      ...
      const top =
        Math.min(itemRect.top, viewportHeight - paddingBottom - submenuRect.height) -
        containerRect.top;
